...
ifeq ($(TARGET_DC),1)
  TARGET_CFLAGS := -DGBI_FLOATS -fno-strict-aliasing -Wall -Os -DTARGET_DC -D_LANGUAGE_C -DTARGET_DC -D_LANGUAGE_C  -Iinclude -Ibuild/us -Ibuild/us/include -Isrc -Isrc/racing -Isrc/ending -I. -DVERSION_US=1 -DF3DEX_GBI=1 -DF3D_OLD=1 -D_LANGUAGE_C=1 -DNON_MATCHING=1 -DAVOID_UB=1 -DAVOID_UB=1 -DGCC=1 -Wall -Wextra -DENABLE_OPENGL -Os -Wno-incompatible-pointer-types -Wno-int-conversion -g3 -fwrapv -Wstrict-overflow=2
  CC_CFLAGS :=
endif
...
# AICA voice driver: regenerate the ADPCM pool (dc_data/adpcm_pool.bin, loaded at
# runtime) + sample table from the source banks (timestamp-gated; only re-runs
# when the banks change). scipy-optional -- a clean checkout builds with python3.
AICA_TOOLS := $(wildcard tools/aica/*.py)

$(BUILD_DIR)/aica_sample_table.c: bin/audiobanks.us.bin bin/audiotables.bin $(AICA_TOOLS)
	PYTHONPATH=tools/aica $(PYTHON) tools/aica/mk64_emit.py \
		--audiobanks bin/audiobanks.us.bin --audiotables bin/audiotables.bin \
		--outdir $(BUILD_DIR) --incdir $(BUILD_DIR) --pool dc_data/adpcm_pool.bin

# emitted together with aica_sample_table.c by the one recipe above; touch so
# they aren't perpetually "older" than the .c (which would force a re-link).
$(BUILD_DIR)/aica_sample_table.h dc_data/adpcm_pool.bin: $(BUILD_DIR)/aica_sample_table.c
	@touch $@
